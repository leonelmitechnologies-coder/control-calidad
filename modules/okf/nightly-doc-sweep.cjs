...
if (fs.existsSync(DICT_DIR)) {
  ...
  if (fs.existsSync(exporter)) {
    const before = fs.existsSync(OKF_OUT)
      ? sh(`find ${OKF_OUT} -name '*.md' | xargs md5sum 2>/dev/null | md5sum`)
      : "";
    sh(
      `node ${exporter} ${DICT_DIR} ${OKF_OUT} --app ${APP} ${process.env.DB_KIND ? `--db-kind "${process.env.DB_KIND}"` : ""} ${process.env.BASE_URI ? `--base-uri "${process.env.BASE_URI}"` : ""}`,
    );
    const after = fs.existsSync(OKF_OUT)
      ? sh(`find ${OKF_OUT} -name '*.md' | xargs md5sum 2>/dev/null | md5sum`)
      : "";
    if (before !== after)
      out.actions.push(
        "OKF bundle regenerated — schema/dictionary changed; commit the updated bundle.",
      );
  }
} else {
  out.actions.push(
    `No data dictionary at ${DICT_DIR} — add the §14d Developer Manual module so OKF can be generated.`,
  );
}
...
if (Number(lastCode) > Number(lastEditOf("'docs/**' 'CHANGELOG.md'"))) {
  out.actions.push(
    "Code changed after the last docs/Changelog edit — review User Manual (§17a) + add a Changelog entry (§17b).",
  );
}
...
if (fs.existsSync("package.json")) {
  ...
  try {
    ...
    const ups = Object.entries(o).map(([n, d]) => ({
      pkg: n,
      cur: d.current,
      latest: d.latest,
    ...
    if (ups.length) {
      ...
      out.actions.push(
        `${ups.length} dependency upgrade(s) available (${majors} major) — RECOMMEND to owner; do not auto-apply (Tier-1).`,
      );
    }
  } catch {
    /* npm outdated returns non-JSON when clean */
  }
}
