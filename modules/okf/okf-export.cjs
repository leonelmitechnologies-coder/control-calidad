...
if (!dictDir || !outDir) { console.error("usage: node okf-export.cjs <dictionary-dir> <out-bundle-dir> [--db-kind X] [--app NAME] [--base-uri URI]"); process.exit(2); }
...
function fm(obj) {                                // minimal YAML frontmatter writer
...
if (!files.length) { console.error(`no *.json dictionary files in ${dictDir}`); process.exit(1); }
const domains = files.map((f) => ({ file: f, ...JSON.parse(fs.readFileSync(path.join(dictDir, f), "utf8")) }));
...
const linkFor = (name) => (tableSlug[name] ? `[${name}](/tables/${tableSlug[name]}.md)` : `\`${name}\``);
...
for (const d of domains) {
  for (const t of d.tables || []) {
    ...
    const colRows = cols.map((c) => {
      ...
      const refs = c.references ? cell(c.references).replace(/\b([a-z_][a-z0-9_]+)\b/gi, (m) => (tableSlug[m] ? linkFor(m) : m)) : "—";
    ...
    const fks = (t.foreignKeys || []).length
      ? (t.foreignKeys).map((fk) => `- ${fk.column || fk.from || "?"} → ${linkFor(fk.references || fk.to || fk.table || "?")}`).join("\n")
      : "none";
    const queries = (t.sampleQueries || []).map((q) => `- ${q.description}: \`${q.sql}\``).join("\n") || "—";
    const body = [
      `# ${t.name}`, "",
      t.purpose || "", "",
...
const indexBody = [
  ...
  ...domains.flatMap((d) => [
    `### ${d.title || d.domainKey}`,
    ...(d.tables || []).map((t) => `- [${t.name}](/tables/${slug(t.name)}.md) — ${(t.purpose || "").split("\n")[0].slice(0, 90)}`),
    "",
  ]),
].join("\n");
fs.writeFileSync(path.join(outDir, "index.md"), fm({ type: "Index", title: `${APP} knowledge bundle`, timestamp: now }) + indexBody);
...
let log = fs.existsSync(logPath) ? fs.readFileSync(logPath, "utf8") : fm({ type: "Log", title: `${APP} bundle history`, timestamp: now }) + `# Update log\n\n`;
fs.writeFileSync(logPath, log.trimEnd() + "\n" + entry + "\n");

console.log(`OKF bundle written to ${outDir}: ${conceptCount} concepts, ${domains.length} domains, + index.md + log.md`);
