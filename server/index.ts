...
app.get("/api/b2c-orders", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const result = await bm
      ...
      .query(`
        SELECT
          ot.OrderID, ot.OrderEntryID,
          o.EnteredDate AS FechaIngreso,
          ot.ShipDate, o.AccountName, ot.ShipBy,
          ...
          CASE
            ...
            THEN REVERSE(LEFT(REVERSE(ot.MappedSKU), CHARINDEX('-', REVERSE(ot.MappedSKU)) - 1))
            ELSE NULL
          ...
          ot.Tracking, ot.Shipment_ID, wl.LocationName
    ...
    for (const row of result.recordset) {
      ...
      if (!ordersMap.has(key)) {
        ordersMap.set(key, {
          OrderID: row.OrderID, OrderEntryID: row.OrderEntryID,
          FechaIngreso: row.FechaIngreso, ShipDate: row.ShipDate,
          AccountName: row.AccountName, ShipBy: row.ShipBy,
          CustomerShippingName: row.CustomerShippingName,
          Qty: 0, CanalVenta: row.CanalVenta, Status: row.Status,
          Tracking: row.Tracking, Shipment_ID: row.Shipment_ID,
          LocationName: row.LocationName, Items: [] as Record<string, unknown>[],
        });
      }
      const order = ordersMap.get(key)!;
      order.Qty = (order.Qty as number) + (row.Qty ?? 0);
      (order.Items as Record<string, unknown>[]).push({
        WebSKU: row.WebSKU, MitSKU: row.MitSKU, LPN: row.LPN,
        Clasificacion: row.Clasificacion, DescripcionProducto: row.DescripcionProducto,
        Qty: row.Qty,
      });
    }
  ...
  try {
    ...
    const ordersResult = await bm
      ...
      .query(`
        SELECT
          o.OrderID,
          ISNULL(c.FullName + ' CID(' + CAST(o.CustomerID AS VARCHAR) + ')', ISNULL(o.AccountName, 'Sin nombre')) AS CustomerName,
          ISNULL(o.Total, 0) AS Total,
          CASE o.OrderStatus
            WHEN 0 THEN 'Cancelled'
            ELSE CASE o.StatusInternal
              WHEN 2 THEN 'Processing' WHEN 4 THEN 'In Transit' WHEN 6 THEN 'Complete'
              WHEN 7 THEN 'Cancelled'  WHEN 8 THEN 'Partial'    WHEN 16 THEN 'In Transit'
              WHEN 17 THEN 'Complete'  ELSE 'Pending'
            END
          END AS Status,
          ISNULL(o.Location, '') AS Location,
          o.EnteredDate, ISNULL(o.EnteredBy, '') AS EnteredBy,
          o.InvoiceDate, o.DueDate, o.CurrencyCode,
        ...
        LEFT JOIN (SELECT OrderID, MIN(BillingAddress) AS BillingAddress FROM OM.OrderShippings GROUP BY OrderID) ship ON o.OrderID = ship.OrderID
        LEFT JOIN (SELECT OrderID, SUM(Qty) AS UnitsOrdered FROM OM.OrderItems GROUP BY OrderID) units ON o.OrderID = units.OrderID
        LEFT JOIN (SELECT OrderID, SUM(Qty) AS UnitsDelivered FROM OM.OrderDeliveryDetails GROUP BY OrderID) delivered ON o.OrderID = delivered.OrderID
        LEFT JOIN (SELECT OrderID, SUM(NotFoundQty) AS NotFound FROM FFM.BatchUserAssignment GROUP BY OrderID) notfound ON o.OrderID = notfound.OrderID
        LEFT JOIN (SELECT OrderID, SUM(Amount * ISNULL(CurrencyValue, 1)) AS Paid FROM OM.OrderPayments GROUP BY OrderID) pay_agg ON o.OrderID = pay_agg.OrderID
    ...
    for (const row of orders) ordersMap.set(row.OrderID as number, { ...row, Items: [] });
