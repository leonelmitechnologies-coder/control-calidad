...
function FormModal({ capa, onClose, onSaved }: FormModalProps) {
  ...
  const [fechaApertura, setFechaApertura] = useState(
    capa?.fecha_apertura
      ? String(capa.fecha_apertura).slice(0, 10)
  ...
  const [porques, setPorques] = useState<string[]>(() => {
    const arr = ["", "", "", "", ""];
    if (capa?.porques) {
      capa.porques.forEach((p) => {
        if (p.orden >= 1 && p.orden <= 5) arr[p.orden - 1] = p.respuesta;
      });
    }
    return arr;
  });
  ...
  const [ishikawa, setIshikawa] = useState<Record<string, string[]>>(() => {
    ...
    if (capa?.ishikawa) {
      ...
      ISHIKAWA_CATS.forEach((cat) => {
        if (map[cat]?.length) init[cat] = [...map[cat], ""];
      });
    }
    return init;
  });
  ...
  const [acciones, setAcciones] = useState<Accion[]>(() =>
    capa?.acciones?.length
      ? capa.acciones.map((a) => ({
          ...a,
          fecha_compromiso: a.fecha_compromiso ? String(a.fecha_compromiso).slice(0, 10) : "",
        }))
  ...
  const [origenes, setOrigenes] = useState<{
    nc: { id: number; label: string }[];
    re: { id: number; label: string }[];
  }>({ nc: [], re: [] });
  ...
  useEffect(() => {
    Promise.all([
      ...
      setOrigenes({
        nc: (Array.isArray(ncs) ? ncs : []).map((r) => ({
          id: r.id,
          label: `NC #${r.id} — ${r.area} / ${r.tipo} (${String(r.fecha ?? "").slice(0, 10)})`,
        })),
        re: (Array.isArray(res) ? res : []).map((r) => ({
          id: r.id,
          label: `RE #${r.id} — ${r.license_plate} (${r.brand ?? ""})`,
        })),
      });
    });
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const ishi =
        metodo === "ishikawa"
          ? ISHIKAWA_CATS.flatMap((cat) =>
              ishikawa[cat]
                .filter((c) => c.trim())
                .map((c) => ({ categoria: cat, causa: c.trim() })),
            )
          : [];
      ...
      if (capa?.id) {
        await apiFetch(`${API_BASE_URL}/api/capas/${capa.id}`, {
          method: "PUT",
        ...
        await apiFetch(`${API_BASE_URL}/api/capas`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
      ...
      >
        <div className="p-6">
          <div className="modal-titulo">
            {capa ? `Editar CAPA #${capa.id}` : "Nueva Acción Correctiva (CAPA)"}
          </div>
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="form-grid">
                ...
                <div>
                  ...
                  <select value={origenId} onChange={(e) => setOrigenId(e.target.value)} required>
                    <option value="">
                      {origenTipo ? "— Seleccionar —" : "— Primero selecciona tipo —"}
                    </option>
              ...
              <div className="flex gap-6 mb-3" style={{ marginTop: "10px" }}>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    ...
                    name="metodo"
                    value="5porques"
                    checked={metodo === "5porques"}
                    onChange={() => setMetodo("5porques")}
                  />
                  5 Porqués
                </label>
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  <input
                    ...
                    name="metodo"
                    value="ishikawa"
                    checked={metodo === "ishikawa"}
                    onChange={() => setMetodo("ishikawa")}
                  />
                  Ishikawa (6M)
                </label>
              </div>
                      ...
                      {ishikawa[cat].map((causa, idx) => (
                        <div key={idx} className="flex gap-1 mb-1">
                          ...
                          <button
                            ...
                            onClick={() => removeIshikawaCausa(cat, idx)}
                          >
                            ✕
                          </button>
                        </div>
            ...
            <div>
              ...
              <div
                className="grid grid-cols-[1fr_140px_120px_110px_28px] gap-1 mb-1 px-1"
...
function DetalleModal({ capaId, onClose, onEdit, onRefresh }: DetalleModalProps) {
  ...
  const eliminar = async () => {
    const ok = await confirm({
      title: "Eliminar CAPA",
      message: "Se eliminará la CAPA y todo su análisis de forma permanente.",
    });
    if (!ok) return;
    try {
  ...
  if (!c)
    return (
      <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
        <div className="bg-white p-8" style={{ color: "#777" }}>
          Cargando...
        </div>
      </div>
    );
  ...
  const analisisHtml = () => {
    if (c.metodo_analisis === "5porques") {
      ...
      if (!filas.length)
        return (
          <p className="vacio" style={{ textAlign: "left", padding: "4px 0" }}>
    ...
    (c.ishikawa ?? []).forEach((r) => {
      if (!catMap[r.categoria]) catMap[r.categoria] = [];
      catMap[r.categoria].push(r.causa);
    });
    return (
      <div className="grid grid-cols-2 gap-3">
        {ISHIKAWA_CATS.map((cat) => (
          <div key={cat} className="card" style={{ padding: "12px" }}>
            <p style={{ fontSize: "11px", fontWeight: "bold", color: "#444", marginBottom: "8px" }}>
              {cat}
            </p>
            {(catMap[cat] ?? []).length ? (
              catMap[cat].map((ca, i) => (
                <div
                  key={i}
                  style={{ fontSize: "13px", padding: "2px 0", borderBottom: "1px solid #f0f0f0" }}
                >
                  {ca}
                </div>
              ))
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <span className={badgeOrigen(c.origen_tipo)}>
                {c.origen_tipo === "nc" ? "NC" : "RE"}
              </span>{" "}
            ...
            <div>
              <span style={{ fontSize: "11px", color: "#aaa", display: "block" }}>Responsable</span>
              {c.responsable}
            </div>
            <div>
              ...
              {metodoLabel[c.metodo_analisis] ?? c.metodo_analisis}
            </div>
            <div>
              <span style={{ fontSize: "11px", color: "#aaa", display: "block" }}>
                Fecha apertura
              </span>
              {String(c.fecha_apertura).slice(0, 10)}
            </div>
            <div>
              <span style={{ fontSize: "11px", color: "#aaa", display: "block" }}>
                Fecha compromiso
              </span>
              {c.fecha_compromiso ? String(c.fecha_compromiso).slice(0, 10) : "—"}
            </div>
            {c.fecha_cierre && (
              <div>
                <span style={{ fontSize: "11px", color: "#aaa", display: "block" }}>
          ...
          <div>
            <div className="seccion-titulo">
              Análisis de Causa Raíz — {metodoLabel[c.metodo_analisis]}
            </div>
          ...
          {pidVerificado && (
            ...
            >
              <p style={{ fontSize: "13px", fontWeight: "500" }}>
                Verificado por (quien cierra la CAPA):
              </p>
              <input
                className="w-full"
                value={verificadoPor}
              ...
              <div className="btn-grupo">
                <button className="btn btn-primario" onClick={confirmarCierre}>
                  Confirmar cierre
                </button>
                <button className="btn btn-secundario" onClick={() => setPidVerificado(false)}>
                  Cancelar
                </button>
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-2 pt-2" style={{ borderTop: "1px solid #e2e2e2" }}>
            {c.estatus === "Abierta" && (
              <button className="btn btn-secundario" onClick={() => cambiarEstatus("En proceso")}>
                En proceso
              </button>
            )}
            {c.estatus !== "Cerrada" && (
              <button className="btn btn-primario" onClick={() => cambiarEstatus("Cerrada")}>
                Cerrar CAPA
              </button>
            )}
            {c.estatus === "Cerrada" && (
              <button className="btn btn-secundario" onClick={() => cambiarEstatus("Abierta")}>
                Reabrir
              </button>
            )}
              ...
              {filtered.map((r) => (
                <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => setDetailId(r.id)}>
                  ...
                  <td onClick={(e) => e.stopPropagation()}>
                    <button className="btn-accion" onClick={() => setDetailId(r.id)}>
                      Ver
                    </button>
                  </td>
                </tr>
