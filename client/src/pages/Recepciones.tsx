...
function buildListQueryKey(page: number, estatus: StatusTab, search: string) {
...
function useTabCounts(allData: RecepcionesResponse | undefined) {
  ...
  const countQuery = useQuery<RecepcionesResponse>({
    ...
    queryFn: () => apiFetch<RecepcionesResponse>("/api/recepciones?page=1&limit=1"),
...
export default function Recepciones() {
  ...
  const fetchSingle = useCallback(
    async (id: number): Promise<Recepcion | null> => {
      try {
        setFetchingId(id);
  ...
  const deleteMutation = useMutation<unknown, Error, number>({
    mutationFn: (id) => apiFetch(`/api/recepciones/${id}`, { method: "DELETE" }),
    onSuccess: () => {
      notify(t("common.success"), "success");
      invalidateAll();
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p style={{ fontSize: 13, color: "#666", marginTop: 2 }}>{t("nav.recepciones")}</p>
        </div>
