...
function useInspecciones() {
  ...
  const getLatest = useCallback(
    (orderId: string): Inspeccion | undefined => {
...
function unitsRemBadge(order: B2BOrder) {
  ...
  if (order.status === "Cancelled")
    return { label: `0/${order.unitsOrdered}`, color: "#c0392b", bg: "#fde8e8" };
  if (order.unitsRemaining > 0)
    return { label: String(order.unitsRemaining), color: "#c0392b", bg: "#fde8e8" };
  ...
  return (
    <div
      style={{
        ...
        borderLeft: `4px solid ${accent ?? "#0d2b4e"}`,
        flex: 1,
        minWidth: 160,
      }}
    >
...
function DetailField({ label, value }: { label: string; value?: string | number | null }) {
  ...
  return (
    <div>
      ...
      <p
        style={{ margin: 0, fontSize: 13, color: "#222", fontWeight: 500, wordBreak: "break-word" }}
      >
        {display}
      </p>
    </div>
...
function InspeccionModal({
  ...
  const canSave =
    estatus !== null && inspector.trim() !== "" && (estatus === "ok" || comentario.trim() !== "");
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        >
          <div>
            <p style={{ margin: 0, fontWeight: 700, fontSize: 15, color: "#0d2b4e" }}>
              Inspección QC
            </p>
            <p style={{ margin: 0, fontSize: 12, color: "#888" }}>
              Orden #{order.orderId} · {order.customer.split(" CID")[0]}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div style={{ padding: "18px 22px" }}>
          ...
          {latest && (
            <div
              style={{
                ...
                padding: "10px 14px",
                borderRadius: 6,
                background: INSP_CONFIG[latest.estatus].bg,
                border: `1px solid ${INSP_CONFIG[latest.estatus].border}`,
              }}
            >
              ...
              <p
                style={{
                  margin: 0,
                  fontWeight: 700,
                  color: INSP_CONFIG[latest.estatus].color,
                  fontSize: 13,
                }}
              >
              ...
              <p style={{ margin: "4px 0 0", fontSize: 11, color: "#aaa" }}>
                Inspector: {latest.inspector} · {new Date(latest.timestamp).toLocaleString("es-MX")}
              </p>
            </div>
          )}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12,
                    cursor: "pointer",
                    border: `2px solid ${active ? cfg.color : "#ddd"}`,
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <textarea
              ...
              rows={3}
              placeholder="Descripción de hallazgos / observaciones..."
              style={{
                width: "100%",
                padding: "8px 12px",
                border: "1px solid #ddd",
                  ...
                  return (
                    ...
                    >
                      ...
                      <p style={{ margin: 0, fontSize: 11, color: "#aaa" }}>
                        {h.inspector} · {new Date(h.timestamp).toLocaleString("es-MX")}
                      </p>
                    </div>
                  );
                })}
      ...
      >
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontWeight: 700, fontSize: 16, color: "#0d2b4e" }}>
              Orden #{order.orderId}
            </span>
            <span
              style={{
        ...
        <div style={{ padding: "22px 24px" }}>
          ...
          <div style={{ display: "flex", gap: 12, marginBottom: 22, flexWrap: "wrap" }}>
            {[
              ...
              {
                l: "Units Remaining",
                v: order.unitsRemaining,
                c: order.unitsRemaining > 0 ? "#c0392b" : "#27ae60",
              },
          ...
          >
            <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: "#0d2b4e" }}>
              ...
              {lpnSearch.trim() ? ` de ${order.items.length}` : ""})
            </p>
              ...
              <tbody>
                ...
                {visibleItems.map((item, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #f0f0f0" }}>
                    ...
                    <td style={{ padding: "8px 10px", color: "#333", maxWidth: 300 }}>
                      {item.description}
                    </td>
                    ...
                    <td
                      style={{
                        ...
                        textAlign: "center",
                        fontWeight: 600,
                        color: item.qtyDelivered === item.qtyOrdered ? "#27ae60" : "#c0392b",
                      }}
                    >
                      {item.qtyDelivered}
                    </td>
                    ...
                    <td style={{ padding: "8px 10px", textAlign: "right", fontWeight: 600 }}>
                      {item.subtotal > 0 ? fmtUSD(item.subtotal) : "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
...
export default function B2BDashboard() {
  ...
  const {
    ...
    queryFn: () =>
      fetch(`/api/b2b-orders?startDate=${apiFrom}&endDate=${apiTo}`).then((r) => {
  ...
  useEffect(() => {
    setPage(1);
  }, [filterStatus, filterCustomer, filterVendedor, filterSKU, filterClasificacion]);

  const customers = useMemo(
    () => Array.from(new Set(orders.map((o) => o.customer.split(" CID")[0]))).sort(),
    [orders],
  );
  const statuses = useMemo(() => Array.from(new Set(orders.map((o) => o.status))).sort(), [orders]);
  const vendedores = useMemo(
    () => Array.from(new Set(orders.map((o) => o.salesRep1).filter(Boolean))).sort(),
    [orders],
  );
  const skus = useMemo(
    () => Array.from(new Set(orders.flatMap((o) => o.items.map((i) => i.sku)))).sort(),
    [orders],
  );
  const clasificaciones = useMemo(() => {
    const skuClasif = (sku: string) => (sku.includes("-") ? sku.split("-").pop()! : "");
    return Array.from(
      new Set(orders.flatMap((o) => o.items.map((i) => skuClasif(i.sku))).filter(Boolean)),
    ).sort();
  }, [orders]);
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((o) => {
      ...
      if (filterClasificacion && !o.items.some((i) => skuClasif(i.sku) === filterClasificacion))
        return false;
      ...
      return (
        ...
        o.items.some(
          (it) =>
            ...
            it.allLpns.toLowerCase().includes(q),
        )
      );
    });
  }, [
  ...
  const chartPorDia = useMemo(() => {
    ...
    for (const o of rawOrders) {
      ...
      byDay.set(dia, {
        ...
        unidades: prev.unidades + (o.UnitsOrdered || 0),
      });
    }
  ...
  const chartStatus = useMemo(() => {
    const by = new Map<string, number>();
    for (const o of rawOrders) {
      by.set(o.Status, (by.get(o.Status) ?? 0) + 1);
    }
    return Array.from(by.entries()).map(([name, value]) => ({
      name,
      value,
      color: STATUS_COLORS[name] ?? "#555",
    }));
  }, [rawOrders]);
  ...
  const totalPending = filtered.filter((o) =>
    ["Pending", "Processing", "Partial"].includes(o.status),
  ).length;
    ...
    <div style={{ fontFamily: "inherit" }}>
      ...
      {(isLoading || isError) && (
        <div
          style={{
            ...
            fontSize: 12,
            fontWeight: 600,
            background: isError ? "#fde8e8" : "#e8f2fb",
            color: isError ? "#c0392b" : "#2980b9",
      ...
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 20 }}>
        ...
        <KpiCard
          label="En Proceso"
          value={totalPending}
          sub="pendientes / parciales"
          accent="#c0711a"
        />
        <KpiCard
          label="Units Ordered"
          value={totalUnits.toLocaleString()}
          sub={`${totalDelivered.toLocaleString()} entregadas`}
          accent="#2980b9"
        />
        <KpiCard
          label="Valor Total"
          value={fmtUSD(totalValue)}
          sub="en órdenes filtradas"
          accent="#8e44ad"
        />
      </div>
      ...
      >
        <div style={{ display: "flex", borderBottom: "1px solid #eee", paddingLeft: 8 }}>
          ...
          <button style={tabStyle("detalle")} onClick={() => setTab("detalle")}>
            Detalle de Órdenes
          </button>
        </div>
      ...
      {tab === "resumen" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            >
              <p style={{ margin: "0 0 16px", fontSize: 13, fontWeight: 700, color: "#0d2b4e" }}>
                Órdenes y Unidades por Día
              </p>
                ...
                <BarChart data={chartPorDia} margin={{ top: 16, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <Bar dataKey="ordenes" name="Órdenes" fill="#0d2b4e" radius={[3, 3, 0, 0]}>
                    <LabelList
                      dataKey="ordenes"
                      position="top"
                      style={{ fontSize: 10, fill: "#0d2b4e", fontWeight: 700 }}
                    />
                  </Bar>
                  <Bar dataKey="unidades" name="Unidades" fill="#2980b9" radius={[3, 3, 0, 0]}>
                    <LabelList
                      dataKey="unidades"
                      position="top"
                      style={{ fontSize: 10, fill: "#2980b9", fontWeight: 700 }}
                    />
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            >
              <p style={{ margin: "0 0 8px", fontSize: 13, fontWeight: 700, color: "#0d2b4e" }}>
          ...
          >
            <p style={{ margin: "0 0 14px", fontSize: 13, fontWeight: 700, color: "#0d2b4e" }}>
            ...
            <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
              {[
                {
                  l: "Units Ordered",
                  v: orders.reduce((a, o) => a + o.unitsOrdered, 0),
                  c: "#0d2b4e",
                },
                {
                  l: "Units Delivered",
                  v: orders.reduce((a, o) => a + o.unitsDelivered, 0),
                  c: "#27ae60",
                },
                {
                  l: "Units Remaining",
                  v: orders.reduce((a, o) => a + o.unitsRemaining, 0),
                  c: "#c0392b",
                },
                ...
                >
                  <p style={{ margin: "0 0 4px", fontSize: 28, fontWeight: 700, color: c }}>
                    {v.toLocaleString()}
                  </p>
                  <p
                    style={{
              ...
              <tbody>
                    ...
                    >
                      ...
                      <td style={{ padding: "10px 12px", color: "#333", maxWidth: 200 }}>
                        <span style={{ display: "block", fontWeight: 600 }}>
                          {order.customer.split(" CID")[0]}
                        </span>
                        <span style={{ fontSize: 10, color: "#aaa" }}>
                          {order.customer.match(/CID\(\d+\)/)?.[0]}
                        </span>
                      </td>
                      <td
                        style={{
                          padding: "10px 12px",
                          color: order.balance < 0 ? "#c0392b" : "#333",
                      ...
                      <td
                        style={{
                          ...
                          fontWeight: 600,
                          color:
                            order.unitsDelivered === order.unitsOrdered ? "#27ae60" : "#c0392b",
                        }}
                      >
                      ...
                      <td style={{ padding: "10px 12px" }}>
                        <InspeccionBadge
                          inspeccion={insp}
                          onClick={() => setInspeccionOrder(order)}
                        />
                      </td>
                      <td style={{ padding: "10px 12px" }}>
                        <button
                ...
                {pageItems.length === 0 && (
                  <tr>
                    <td
                      colSpan={12}
                      style={{ padding: 40, textAlign: "center", color: "#aaa", fontSize: 13 }}
                    >
                      Sin resultados
                    </td>
                  </tr>
      ...
      {detailOrder && (
        <DetailModal
          ...
          onInspeccionar={() => {
            setInspeccionOrder(detailOrder);
            setDetailOrder(null);
          }}
        />
      )}
