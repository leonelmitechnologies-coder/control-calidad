...
function useInspecciones() {
  ...
  const getLatest = useCallback(
    (ordenId: number): Inspeccion | undefined => {
...
function InspeccionModal({
  ...
  const handleSave = () => {
    if (!estatus) {
      setError("Selecciona un estatus.");
      return;
    }
    if (!inspector.trim()) {
      setError("Escribe tu nombre como inspector.");
      return;
    }
    if (estatus !== "ok" && !comentario.trim()) {
      setError("El comentario es obligatorio cuando hay problema o no salió.");
      return;
    }
    onSave({
      estatus,
  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        >
          <div>
            <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: "#0d2b4e" }}>
              Inspección — Orden #{order.OrderID}
            </h2>
            <p style={{ margin: "2px 0 0", fontSize: 11, color: "#888" }}>
              {order.AccountName} · {order.CustomerShippingName}
            </p>
          </div>
          {ultima && (
            <span
              style={{
                background: INSP_CONFIG[ultima.estatus].bg,
                color: INSP_CONFIG[ultima.estatus].color,
          ...
          <div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {(
                  Object.entries(INSP_CONFIG) as [
                    InspeccionEstatus,
                    (typeof INSP_CONFIG)[InspeccionEstatus],
                  ][]
                ).map(([k, cfg]) => (
                  <button
                    key={k}
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <textarea
                ...
                rows={3}
                placeholder={
                  estatus === "con_problemas"
                    ? "Describe el problema observado y qué acción se tomó..."
                    : estatus === "no_salio"
                      ? "Describe por qué no salió la orden..."
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <button
                ...
                style={{ fontSize: 12, marginBottom: fotos.length ? 10 : 0 }}
              >
                + Agregar fotos
              </button>
              {fotos.length > 0 && (
            ...
            {error && (
              <p style={{ fontSize: 12, color: "#c0392b", margin: "0 0 10px", fontWeight: 600 }}>
                {error}
              </p>
            )}
          </div>
                    ...
                    >
                      ...
                      >
                        ...
                        <span style={{ fontSize: 11, color: "#888" }}>
                          {new Date(h.timestamp).toLocaleString("es-MX")} · {h.inspector}
                        </span>
                      </div>
                      {h.comentario && (
        ...
        >
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button type="button" onClick={handleSave} className="btn btn-primario">
              Guardar Inspección
            </button>
          </div>
        </div>
...
function detectMarketplace(accountName: string): "ml" | "amazon" | "walmart" | "" {
  ...
  if (
    ...
    lower.includes("berojov") ||
    lower.includes("blow") ||
    lower.includes("lutema") ||
    lower.includes("apantallate") ||
    lower.includes("autobot mx") ||
    lower.includes("remotes")
  )
    return "ml";
  ...
  return (
    <div
      style={{
        ...
        border: "1px solid #e2e2e2",
        padding: "18px 22px",
        borderTop: `3px solid ${color ?? "#0d2b4e"}`,
        flex: 1,
        minWidth: 160,
      }}
    >
        ...
        <div style={{ padding: "20px 24px" }}>
          ...
          >
            ...
            >
              ...
              >
                {visibleItems.length}
                {lpnSearch.trim() ? ` / ${order.Items.length}` : ""}
              </span>
            </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: "#0d2b4e", color: "#fff" }}>
                  {["LPN", "Web SKU", "MIT SKU", "Cant.", "Clasificación", "Descripción"].map(
                    (h) => (
                      <th
                        key={h}
                        style={{
                ...
                {visibleItems.map((item, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: "8px 12px", fontFamily: "monospace", fontSize: 11 }}>
                      <span style={{ background: "#f0f4f9", borderRadius: 3, padding: "1px 6px" }}>
                        {item.WebSKU}
                      </span>
                    </td>
                    <td style={{ padding: "8px 12px", fontFamily: "monospace", fontSize: 11 }}>
...
export default function B2CDashboard() {
  ...
  const clients = useMemo(
    () => [...new Set(allOrders.map((o) => o.AccountName))].sort(),
    [allOrders],
  );
  const channels = useMemo(
    () => [...new Set(allOrders.map((o) => o.CanalVenta))].sort(),
    [allOrders],
  );
  const vendedores = useMemo(
    () => [...new Set(allOrders.map((o) => o.ShipBy).filter(Boolean))].sort(),
    [allOrders],
  );
  const skus = useMemo(
    () =>
      [...new Set(allOrders.flatMap((o) => o.Items.map((i) => i.WebSKU)).filter(Boolean))].sort(),
    [allOrders],
  );
  const clasificaciones = useMemo(
    () =>
      [
        ...new Set(allOrders.flatMap((o) => o.Items.map((i) => i.Clasificacion)).filter(Boolean)),
      ].sort(),
    [allOrders],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return allOrders.filter((o) => {
      ...
      if (filterClasificacion && !o.Items.some((i) => i.Clasificacion === filterClasificacion))
        return false;
      if (filterSKU.length > 0 && !o.Items.some((i) => filterSKU.includes(i.WebSKU))) return false;
      if (sq) {
        const hay = [
          o.OrderID,
          String(o.OrderEntryID),
          o.AccountName,
          o.ShipBy,
          o.CustomerShippingName,
          o.CanalVenta,
          o.Status,
          o.Tracking,
  ...
  const kpiEnProceso = allOrders.filter((o) =>
    ["Pending", "paid", "Unshipped"].includes(o.Status),
  ).length;
  const kpiListos = allOrders.filter((o) => o.Status === "ready_to_print").length;
  const kpiEnviados = allOrders.filter((o) =>
    ["Shipped", "shipped", "Delivered", "delivered"].includes(o.Status),
  ).length;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 13, color: "#666", margin: 0 }}>
            ...
            {isLoading && (
              <span style={{ marginLeft: 10, color: "#2980b9", fontSize: 11, fontWeight: 700 }}>
                Cargando...
              </span>
            )}
            {isError && (
              <span style={{ marginLeft: 10, color: "#c0392b", fontSize: 11, fontWeight: 700 }}>
                Error al conectar con BinManager
              </span>
            )}
          </p>
        </div>
        ...
        {(
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => {
                setFilterMarketplace(active ? "" : key);
                setPage(1);
              }}
              style={{
                padding: "7px 18px",
      ...
      {activeTab === "resumen" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div style={{ background: "#fff", border: "1px solid #e2e2e2", padding: "18px 20px" }}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={chartPorDia} barGap={4} barSize={14}>
                ...
                <Tooltip
                  contentStyle={{ fontSize: 12, border: "1px solid #e2e2e2", borderRadius: 2 }}
                  cursor={{ fill: "#f0f4f9" }}
                />
                ...
                <Bar dataKey="unidades" name="Unidades" fill="#5a9fd4" radius={[2, 2, 0, 0]}>
                  <LabelList
                    dataKey="unidades"
                    position="top"
                    style={{ fontSize: 10, fill: "#5a9fd4", fontWeight: 700 }}
                  />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={Math.max(200, chartSKU.length * 30)}>
              <BarChart data={chartSKU} layout="vertical" barSize={16}>
                ...
                <YAxis
                  type="category"
                  dataKey="sku"
                  tick={{ fontSize: 11, fill: "#333", fontFamily: "monospace" }}
                  axisLine={false}
                  tickLine={false}
                  width={200}
                />
                ...
                <Bar dataKey="unidades" name="Unidades" fill="#0d2b4e" radius={[0, 2, 2, 0]}>
                  <LabelList
                    dataKey="unidades"
                    position="right"
                    style={{ fontSize: 11, fill: "#0d2b4e", fontWeight: 700 }}
                  />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          ...
          <div style={{ background: "#fff", border: "1px solid #e2e2e2", padding: "18px 20px" }}>
            ...
            >
              Unidades por Clasificación
            </div>
            <ResponsiveContainer width="100%" height={260}>
              <PieChart>
                <Pie
                  data={chartClasificacion}
                  cx="50%"
                  cy="42%"
                  innerRadius={55}
                  outerRadius={85}
                  ...
                  label={false}
                >
                  {chartClasificacion.map((entry, i) => (
                    <Cell key={i} fill={entry.color} />
                  ))}
                </Pie>
                <Tooltip
              ...
              <BarChart data={chartVendedor} layout="vertical" barSize={16}>
                ...
                <Tooltip
                  content={({ active, payload }) => {
                    ...
                    return (
                      ...
                      >
                        <div style={{ fontWeight: 700, color: "#0d2b4e", marginBottom: 2 }}>
                          {d.full}
                        </div>
                ...
                <Bar dataKey="ordenes" name="Órdenes" fill="#2980b9" radius={[0, 2, 2, 0]}>
                  <LabelList
                    dataKey="ordenes"
                    position="right"
                    style={{ fontSize: 11, fill: "#2980b9", fontWeight: 700 }}
                  />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
                    ...
                    >
                      ...
                      <td style={{ padding: "10px 12px", whiteSpace: "nowrap" }}>
                        <div style={{ fontSize: 11, color: "#555", fontWeight: 500 }}>
                          {fmtRelative(order.FechaIngreso)}
                        </div>
                        <div style={{ fontSize: 10, color: "#aaa", marginTop: 2 }}>
                          {fmtDate(order.FechaIngreso)}
                        </div>
                      </td>
              ...
              <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12,
                    border: "1px solid #d0d0d0",
                    background: page === totalPages ? "#f4f4f4" : "#fff",
      ...
      {inspOrder && (
        <InspeccionModal
          ...
          onSave={(entry) => {
            saveInsp(inspOrder.OrderEntryID, entry);
            setInspOrder(null);
          }}
