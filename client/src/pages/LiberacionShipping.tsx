...
function badgeEstatusCarga(estatus: string) {
  switch (estatus) {
    case "Cargada y enviada":
      return "badge badge-cerrada";
    case "Cargada y no enviada":
      return "badge badge-proceso";
    case "Pendiente en inspección":
...
function FormModal({ record, onClose, onSaved }: FormModalProps) {
  ...
  const [fecha, setFecha] = useState(
    record?.fecha ? String(record.fecha).slice(0, 10) : new Date().toISOString().slice(0, 10),
  );
  ...
  const [fotos, setFotos] = useState<Record<FotoKey, File | null>>({
    contenedor_vacio: null,
    contenedor_cargado: null,
    caja_sellada: null,
    placas: null,
    manifiesto: null,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (tipoEnvio === "B2C" && !paqueteria) {
      notify("Selecciona la paquetería para envíos B2C.", "error");
      return;
    }
    ...
    try {
      const body = {
        ...
        paqueteria: tipoEnvio === "B2C" ? paqueteria : "",
        tipo_orden: tipoOrden,
        numero_contenedor: numContenedor,
        numero_sello: numSello,
        cantidad_pallets: parseInt(cantPallets) || 0,
        cantidad_manifiesto: man,
        cantidad_fisica: fis,
        estado,
        cantidad_diferencia: diferencia,
        resultado_inspeccion: resultadoInsp,
        estatus_carga: estatusCarga,
      ...
      let id: number;
      if (record?.id) {
        await apiFetch(`${API_BASE_URL}/api/liberacion-shipping/${record.id}`, {
          method: "PUT",
        ...
        const nuevo = await apiFetch<{ id: number }>(`${API_BASE_URL}/api/liberacion-shipping`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
      ...
      for (const { key } of FOTO_KEYS) {
        ...
        if (file) {
          ...
          fd.append("foto", file);
          await apiFetch(`${API_BASE_URL}/api/liberacion-shipping/${id}/${key}`, {
            method: "POST",
            body: fd,
          });
        }
  ...
  const setFotoKey = (key: FotoKey) => (f: File | null) =>
    setFotos((prev) => ({ ...prev, [key]: f }));
      ...
      <div className="bg-white w-full max-w-2xl my-8" style={{ border: "1px solid #e2e2e2" }}>
        <div className="p-6">
          <div className="modal-titulo">
            {record ? "Editar Liberación Shipping" : "Nueva Liberación Shipping"}
          </div>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="form-grid">
              ...
              <div>
                <label>
                  No. de Orden <span style={{ color: "#d00" }}>*</span>
                </label>
                <input
                  type="text"
                  className="w-full"
              ...
              <div>
                <label>
                  Hora de Inicio <span style={{ color: "#d00" }}>*</span>
                </label>
                <input
                  type="time"
                  className="w-full"
              ...
              <div>
                <label>
                  Hora de Fin <span style={{ color: "#d00" }}>*</span>
                </label>
                <input
                  type="time"
                  className="w-full"
            ...
            <div className="form-grid">
              <div>
                <label>
                  Destino <span style={{ color: "#d00" }}>*</span>
                </label>
                <select
                  className="w-full"
                  value={destino}
              ...
              <div>
                <label>
                  Tipo de Envío <span style={{ color: "#d00" }}>*</span>
                </label>
                <select
                  className="w-full"
                  value={tipoEnvio}
                  onChange={(e) => {
                    ...
                    if (e.target.value !== "B2C") setPaqueteria("");
                  }}
                  required
                >
                  <option value="">— Seleccionar —</option>
              ...
              {tipoEnvio === "B2C" && (
                <div className="full">
                  <label>
                    Paquetería <span style={{ color: "#d00" }}>*</span>
                  </label>
                  <select
                    className="w-full"
                    value={paqueteria}
              ...
              <div>
                <label>
                  Tipo de Orden <span style={{ color: "#d00" }}>*</span>
                </label>
                <select
                  className="w-full"
                  value={tipoOrden}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full"
                  value={numSello}
                  onChange={(e) => setNumSello(e.target.value)}
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full"
                  value={cantPallets}
                  onChange={(e) => setCantPallets(e.target.value)}
                />
              </div>
            </div>
            ...
            <div className="form-grid">
              <div>
                <label>
                  Cant. en Manifiesto <span style={{ color: "#d00" }}>*</span>
                </label>
                <input
                  type="number"
                  min="0"
                  className="w-full"
                  value={cantManifiesto}
                  onChange={(e) => setCantManifiesto(e.target.value)}
                  required
                />
              </div>
              <div>
                <label>
                  Cant. Física <span style={{ color: "#d00" }}>*</span>
                </label>
                <input
                  type="number"
                  min="0"
              ...
              <div>
                ...
                <input
                  ...
                  style={{
                    background: "#f8f8f8",
                    fontWeight: "bold",
                    color: estado ? ESTADO_COLOR[estado] : "#333",
                  }}
                  value={estado}
                  readOnly
                />
              ...
              <div>
                ...
                <input
                  ...
                  value={estado && man !== fis ? diferencia : ""}
                  readOnly
                />
              </div>
            </div>
            ...
            <div className="form-grid">
              <div>
                <label>
                  Resultado <span style={{ color: "#d00" }}>*</span>
                </label>
                <select
                  className="w-full"
                  value={resultadoInsp}
                  onChange={(e) => setResultadoInsp(e.target.value)}
                  required
                >
                  <option value="">— Seleccionar —</option>
                  {RESULTADOS.map((r) => (
              ...
              <div>
                <label>
                  Estatus de Carga <span style={{ color: "#d00" }}>*</span>
                </label>
                <select
                  className="w-full"
                  value={estatusCarga}
                  onChange={(e) => setEstatusCarga(e.target.value)}
                  required
                >
                  <option value="">— Seleccionar —</option>
                  {ESTATUS_CARGA.map((s) => (
...
function DetailModal({ record: r, onClose, onEdit, onDeleted }: DetailModalProps) {
  ...
  const eliminar = async () => {
    const ok = await confirm({
      ...
      message: "¿Eliminar esta liberación? Se borrarán las fotos adjuntas.",
    });
    if (!ok) return;
    try {
      await apiFetch(`${API_BASE_URL}/api/liberacion-shipping/${r.id}`, { method: "DELETE" });
      notify("Liberación eliminada.", "success");
      onDeleted();
    } catch (err: any) {
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-sm">
            ...
            <div>
              ...
              <span
                className={
                  r.resultado_inspeccion === "Aceptado"
                    ? "badge badge-cerrada"
                    : "badge badge-rechazada"
                }
              >
                {r.resultado_inspeccion || "—"}
              </span>
            </div>
            <div>
              <span style={{ fontSize: "11px", color: "#aaa", display: "block" }}>
            ...
            {FOTO_KEYS.map(({ key, label }) => {
              ...
              return (
                <div key={key}>
                  ...
                  <a
                    href={`${API_BASE_URL}/uploads/shipping/${filename}`}
                    target="_blank"
                    rel="noreferrer"
                  >
                    <img
                      src={`${API_BASE_URL}/uploads/shipping/${filename}`}
                      alt={label}
                      style={{
                        width: "100%",
                        height: "128px",
            ...
            <tbody>
              {records.map((r, i) => (
                <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => setDetail(r)}>
                  ...
                  <td>{r.tipo_envio}</td>
                  <td>{r.tipo_orden}</td>
                  <td style={{ fontWeight: "bold", color: ESTADO_COLOR[r.estado ?? ""] ?? "#333" }}>
                    {r.estado || "—"}
                  </td>
                  <td>
                    <span
                  ...
                  <td>
                    <span className={badgeEstatusCarga(r.estatus_carga)}>
                      {r.estatus_carga || "—"}
                    </span>
                  </td>
                  <td>{r.inspector || "—"}</td>
                  <td className="whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                    <button className="btn-accion" onClick={() => setDetail(r)}>
                      Ver
                    </button>{" "}
