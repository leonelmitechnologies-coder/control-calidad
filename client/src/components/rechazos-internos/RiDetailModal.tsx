        ...
        >
          ...
          >
            <div>
              ...
              >
                {ri ? `${t("rechazos_internos.title")} #${ri.id}` : t("rechazos_internos.title")}
              </h2>
              {ri && (
            ...
            {ri && (
              <>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  <dl
                    className="grid"
                    style={{ gridTemplateColumns: "repeat(3, 1fr)", gap: "12px 24px" }}
                  >
                    <Field
                      label={t("rechazos_internos.form.fecha_registro")}
                      value={formatDate(ri.fecha_registro, "dd/MM/yyyy")}
                    />
                    <Field
                      label={t("rechazos_internos.form.license_plate")}
                      value={ri.license_plate}
                    />
                    ...
                    {ri.descripcion && (
                      <div style={{ gridColumn: "1 / -1" }}>
                        <Field
                          label={t("rechazos_internos.form.descripcion")}
                          value={ri.descripcion}
                        />
                      </div>
                    )}
                  </dl>
                ...
                <div style={{ marginBottom: 24 }}>
                    ...
                    >
                      ...
                      {Number(ri.costo_no_calidad) !== mappingEntry.costo && (
                        <p style={{ marginTop: 6, fontSize: 12, color: "#856404" }}>
                          Costo registrado ({formatCurrency(Number(ri.costo_no_calidad))}) difiere
                          del mapeo estándar (modificación manual).
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  <dl
                    className="grid"
                    style={{ gridTemplateColumns: "repeat(2, 1fr)", gap: "12px 24px" }}
                  >
                    ...
                    <div style={{ gridColumn: "1 / -1" }}>
                      <Field
                        label={t("rechazos_internos.form.actividad_realizar")}
                        value={ri.actividad_realizar}
                      />
                    </div>
                  </dl>
                </div>
                ...
                <div style={{ marginBottom: 24 }}>
                  ...
                  >
                    <Field
                      label={t("rechazos_internos.form.origen_hallazgo")}
                      value={ri.origen_hallazgo}
                    />
                    ...
                    <Field
                      ...
                      value={
                        <span
                          className={`badge badge-${ri.estatus === "Abierto" ? "abierta" : "cerrada"}`}
                        >
                          {ri.estatus ?? "Abierto"}
                        </span>
                      }
                    ...
                    {ri.observaciones && (
                      <div style={{ gridColumn: "1 / -1" }}>
                        <Field
                          label={t("rechazos_internos.form.observaciones")}
                          value={ri.observaciones}
                        />
                      </div>
                    )}
                  </dl>
                ...
                {images.length > 0 && (
                  <div style={{ marginBottom: 24 }}>
                    <SectionTitle
                      label={`${t("rechazos_internos.form.fotos")} (${images.length})`}
                    />
                ...
                <div style={{ borderTop: "1px solid #e2e2e2", paddingTop: 14 }}>
                  <dl
                    className="grid"
                    style={{ gridTemplateColumns: "repeat(2, 1fr)", gap: "8px 24px" }}
                  >
                    <div>
                      <dt
                        style={{
                      ...
                      <dd style={{ fontSize: 12, color: "#555", margin: 0 }}>
                        {ri.registrado_por}
                      </dd>
                    </div>
                    <div>
                      ...
                      <dd style={{ fontSize: 12, color: "#555", margin: 0 }}>
                        {formatDate(ri.created_at, "dd/MM/yyyy HH:mm")}
                      </dd>
                    </div>
                  </dl>
                </div>
