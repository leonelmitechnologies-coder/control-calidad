      ...
      {open && (
        ...
        >
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {filtered.length === 0 ? (
              <div
                style={{ padding: "10px 12px", fontSize: 12, color: "#aaa", fontStyle: "italic" }}
              >
                Sin resultados
              </div>
            ) : (
  ...
  return (
    <div ref={containerRef} style={{ position: "relative" }}>
      ...
      >
        ...
        <span style={{ color: "#999", fontSize: 10, marginLeft: 4, flexShrink: 0 }}>
          {open ? "▲" : "▼"}
        </span>
      </div>
          ...
          {options.map((opt) => {
            ...
            return (
              <div
                ...
                onMouseEnter={(e) => {
                  if (!checked) (e.currentTarget as HTMLDivElement).style.background = "#f8f9fa";
                }}
                onMouseLeave={(e) => {
                  if (!checked) (e.currentTarget as HTMLDivElement).style.background = "#fff";
                }}
              >
                <div
                  style={{
                    width: 16,
                    height: 16,
                    border: `2px solid ${checked ? "#0d2b4e" : "#ccc"}`,
                    borderRadius: 3,
                    background: checked ? "#0d2b4e" : "#fff",
                    flexShrink: 0,
                    display: "flex",
                    alignItems: "center",
...
type FormErrors = Partial<
  Record<keyof Omit<ReFormData, "problems" | "corrective_actions">, string>
> & {
...
export default function ReForm({
  ...
  useEffect(() => {
    ...
    if (isEditing && data) {
      ...
      setForm({
        ...
        problems:
          probs.length > 0
            ? probs.map((p) => ({ descripcion: p.descripcion }))
            : [{ descripcion: "" }],
  ...
  const handleToggleDept = useCallback((dept: string) => {
    setForm((f) => {
      ...
      if (isActive) {
        return {
          ...f,
          corrective_actions: f.corrective_actions.filter((ca) => ca.departamento !== dept),
        };
      }
      return {
        ...f,
        corrective_actions: [...f.corrective_actions, { departamento: dept, orden: 1, accion: "" }],
      };
    });
  }, []);
  ...
  const handleDeptActionChange = useCallback((globalIdx: number, value: string) => {
    setForm((f) => ({
      ...f,
      corrective_actions: f.corrective_actions.map((ca, i) =>
        i === globalIdx ? { ...ca, accion: value } : ca,
      ),
    }));
  }, []);
    ...
    setForm((f) => {
      ...
      return {
        ...
        corrective_actions: [
          ...
          { departamento: dept, orden: count + 1, accion: "" },
        ],
      };
    });
  }, []);
  ...
  const title = isEditing ? `Editar Rechazo Externo #${data?.id ?? ""}` : "Nueva Recepción";
  ...
  return createPortal(
    ...
    >
      ...
      <div
        ...
        style={{ maxWidth: 720, background: "#fff", border: "1px solid #e2e2e2" }}
      >
        {/* Header */}
        <div
          className="flex items-center justify-between"
          style={{ padding: "16px 24px", borderBottom: "2px solid #0d2b4e" }}
        >
          <h2
            id="re-form-title"
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div style={{ padding: "20px 24px" }}>
            ...
            <div className="form-grid" style={{ marginBottom: 24 }}>
              <div className="form-group">
                <label>
                  Return Order <span style={{ color: "#c0392b" }}>*</span>
                </label>
                <input
                  type="text"
              ...
              <div className="form-group">
                <label>
                  Classification <span style={{ color: "#c0392b" }}>*</span>
                </label>
                <SearchableSelect
                ...
                {errors.classification && (
                  <span className="form-error">{errors.classification}</span>
                )}
              </div>
              ...
              <div className="form-group">
                <label>
                  Plant Entry <span style={{ color: "#c0392b" }}>*</span>
                </label>
                <input
                  type="datetime-local"
              ...
              <div className="form-group full">
                <label>
                  Processed By <span style={{ color: "#c0392b" }}>*</span>
                </label>
                <MultiSelect
              ...
              {form.problems.map((p, idx) => (
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    />
                    {errors.problems?.[idx] && (
                      <span className="form-error">{errors.problems[idx]}</span>
                    )}
                  </div>
                  <button
                    type="button"
              ...
              <div>
                {activeDepts.map((dept) => {
                  ...
                  return (
                    <div
                      key={dept}
                      style={{ border: "1px solid #0d2b4e", marginBottom: 8, background: "#fff" }}
                    >
                      <p
                        style={{
                          fontSize: 11,
          ...
          <div
            ...
            style={{ padding: "14px 24px", borderTop: "1px solid #e2e2e2" }}
          >
