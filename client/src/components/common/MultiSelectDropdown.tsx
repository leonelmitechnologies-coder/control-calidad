...
export function MultiSelectDropdown({ options, value, onChange, placeholder = 'Seleccionar...', style }: Props) {
    ...
    <div ref={ref} style={{ position: 'relative', flex: 1, minWidth: 130, ...style }}>
      <button
        ...
        style={{
          ...
          fontSize: 13, background: '#fff', textAlign: 'left', cursor: 'pointer',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
          color: value.length ? '#222' : '#999',
        }}
      >
        <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{label}</span>
        <span style={{ marginLeft: 6, fontSize: 10, color: '#666', flexShrink: 0 }}>{open ? '▲' : '▼'}</span>
      </button>
      ...
      {open && (
        <div style={{
          ...
          boxShadow: '0 4px 12px rgba(0,0,0,0.15)', minWidth: '100%', maxWidth: 300,
          marginTop: 3,
        }}>
          ...
          {value.length > 0 && (
            <div style={{ padding: '6px 12px', borderTop: '1px solid #eee' }}>
              <button
                type="button"
                onClick={() => onChange([])}
                style={{ fontSize: 11, color: '#c0392b', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
              >
